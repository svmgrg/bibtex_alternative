...
\end{verbatim}
  ...
  The above error message is, by design, ugly and execessively intrusive so as to attract user's attention to the errors. Once the user solves the corresponding error, the message will disappear.
  ...
  \end{minted}
  This will only show the succinct warning message on the terminal and in the log files. (However, we do not recommend using the \texttt{suppress\_errors} flag, since the errors generated by it are very easy to miss.)
