...
\end{verbatim}
  ...
  In case the user does not want to see such verbose messages in the final PDF document, they can load the \dumbib\ package as follows:
  \begin{minted}{latex}
    \usepackage[error_mode]{dumbib}
  \end{minted}
  In this mode, \dumbib\ will not show any errors in the output PDF, and instead raise succinct error messages on the terminal, i.e.\ it will break the \LaTeX\ processing and require the user to type \texttt{<return>} to continue.
