...
\end{verbatim}
  ...
  \end{table}
  \\
  where the \texttt{<author>} and \texttt{<year>} fields correspond to those that were specified during the \dumbib\ database creation. (If you use a combination other than the ones described above, or cite a reference which has not been declared to the \dumbib\ database a priori, \dumbib\ will raise an error.)
  
  As an example, consider the the snippet
  \begin{minted}{latex}
